% TEEN Protocol Simulation with Visualization and Performance Metrics in MATLAB
...
% Sensed Attribute Parameters
attribute_mean = 50;
attribute_std = 20;

% Thresholds
hard_threshold = 55; % Hard threshold (HT)
soft_threshold = 5;  % Soft threshold (ST)
...
for i = 1:n
    ...
    S(i).data_value = 0;            % Last sensed data value
    S(i).data_sent = false;         % Flag to check if data was sent in current round
end
...
for r = 1:max_rounds
    ...
    for i = 1:n
        if S(i).E > 0
            ...
            S(i).data_sent = false; % Reset data sent flag
        else
            S(i).type = 'D';
        end
    end
    ...
    % Cluster Formation and Threshold Broadcasting
    for i = 1:n
        if strcmp(S(i).type, 'N') && S(i).E > 0
            ...
            % Energy consumption for receiving threshold broadcast
            S(i).E = S(i).E - E_elec * k;
            % Cluster head energy for threshold broadcast
            ch = min_dis_cluster;
            if S(ch).E > 0
                S(ch).E = S(ch).E - E_elec * k;
            end
        end
    end
    
    %% Data Transmission Based on Thresholds
    for i = 1:n
        if S(i).E > 0 && strcmp(S(i).type, 'N')
            % Sense the attribute
            sensed_value = attribute_mean + attribute_std * randn;
            % Check if it meets the hard threshold
            if sensed_value >= hard_threshold
                % Check if the change is significant (soft threshold)
                if abs(sensed_value - S(i).data_value) >= soft_threshold || ~S(i).data_sent
    ...
    for ch = cluster_heads
        if S(ch).E > 0
            if any([S([S.cluster] == ch).data_sent]) || S(ch).data_sent
                % Cluster head aggregates data and sends to base station
    ...
    grid on;
    axis([0 xm 0 ym]);
    title(sprintf('TEEN Protocol - Round %d', r));
    xlabel('X Position (m)');
    ylabel('Y Position (m)');
    ...
    % Draw lines from member nodes to their cluster heads (only if data was sent)
    for i = 1:n
        if strcmp(S(i).type, 'N') && S(i).E > 0 && S(i).cluster > 0 && S(i).data_sent
            ch = S(i).cluster;
            if S(ch).E > 0
    ...
    % Draw lines from cluster heads to base station (only if data was sent)
    for ch = cluster_heads
        if S(ch).E > 0 && (any([S([S.cluster] == ch).data_sent]) || S(ch).data_sent)
